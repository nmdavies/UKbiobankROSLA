...
global ols_covariates = "mob_I_1-mob_I_12 rosla int_rosla cov_male cov_age3 rosla rosla_post"
global iv_covariates = "mob_I_1-mob_I_12 int_mob_I_2-int_mob_I_12 rosla int_rosla cov_male cov_age3 rosla rosla_post"
...
args gender gender_desc
	...
	gen cov_age3=((yob-2008)*12)^3
	
	//Rescale intelligence to be in SD units
	replace out_intell=out_intell/2.108772-6.163124
	
	//Run baseline regressions just using actual schooling (more than 15 years):
	reg out_phys_v_act more_edu_15 ${ols_covariates} if (rosla)^2<(${bandwidth})^2 [pweight=weight], cluster(mobi)
	regsave more_edu_15 using "results/CR_outcome_act_exposure_`gender_desc'", replace detail(all) pval ci
	
	//Reduced form
	reg out_phys_v_act post_reform ${iv_covariates} if (rosla)^2<(${bandwidth})^2 & more_edu_15!=. [pweight=weight], cluster(mobi)
	regsave post_reform using "results/CR_outcome_reduced_form_`gender_desc'", replace detail(all) pval ci

	//Instrumental variable regressions			
	ivreg2 out_phys_v_act (more_edu_15=post_reform) ${iv_covariates} if (rosla)^2<(${bandwidth})^2  & more_edu_15!=.  [pweight=weight], endog(more_edu_15) cluster(mobi) partial(${iv_covariates})
	regsave more_edu_15 using "results/CR_outcome_IV_`gender_desc'", replace detail(all) pval ci
	
	//Repeat these regression for all the other outcomes:
	ds out_*
	foreach i in `r(varlist)'{
		reg `i' more_edu_15 ${ols_covariates} if (rosla)^2<(${bandwidth})^2  [pweight=weight], cluster(mobi)
		regsave more_edu_15 using "results/CR_outcome_act_exposure_`gender_desc'", append detail(all) pval ci		
		
		reg `i' post_reform ${iv_covariates} if (rosla)^2<(${bandwidth})^2  & more_edu_15!=. [pweight=weight], cluster(mobi)
		regsave  post_reform using "results/CR_outcome_reduced_form_`gender_desc'", append detail(all) pval ci
		
		ivreg2 `i' (more_edu_15=post_reform) ${iv_covariates} if (rosla)^2<(${bandwidth})^2 [pweight=weight], endog(more_edu_15) cluster(mobi) partial(${iv_covariates}) nocollin 
		regsave more_edu_15 using "results/CR_outcome_IV_`gender_desc'", append detail(all) pval ci
		}
end
